...
const page = () => {
    ...
    const FetchAllOrders = async () => {
        try {
            ...
            const { data } = await axios.get("https://apiv2.shiprocket.in/v1/external/orders", {
                headers: {
    ...
    const SingleOrderDataGet = async () => {
        try {
            const { data } = await axios.get(`https://apiv2.shiprocket.in/v1/external/orders/show/${'434366715'}`, {
                headers: {
                    Authorization: "Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..7LPVnO7pFPwHQIn8ondKGdQhfvrPhxY1t_ANStXq1Xo" || ""
                }
            })
    ...
    return (
        <div>
            ...
            <div >
                <button onClick={() => FetchAllOrders()} style={{ border: "1px solid white", padding: "10px ", margin: "10px 20px" }}>Fetch All Orders</button>
                <button onClick={() => SingleOrderDataGet()} style={{ border: "1px solid white", padding: "10px ", margin: "10px 20px" }}>Single Order Data</button>
                <button onClick={() => TrackingData()} style={{ border: "1px solid white", padding: "10px ", margin: "10px 20px" }}>Tracking </button>
            </div>
